...
# -*- coding: utf-8 -*-
"""
调试app.py生成的请求格式，验证是否符合ScientificReviewer-7B要求
"""
...
def debug_request_format():
    ...
    # 创建PaperRequest对象
    paper_request = PaperRequest.from_dict(test_data)
    ...
    # 1. 查看处理后的论文内容
    print("1. 处理后的论文内容:")
    print(f"   长度: {len(paper_content)} 字符")
    print(f"   内容预览: {paper_content[:200]}...")
    print()
    ...
    prompt = vllm_service._build_peer_review_prompt(paper_content, review_query)
    print("2. 构建的prompt:")
    print(f"   长度: {len(prompt)} 字符")
    print(f"   内容预览: {prompt[:300]}...")
    print()
    
    # 3. 查看生成的VllmRequest
    from models.vllm_models import VllmRequest, VllmMessage
    vllm_request = VllmRequest(
        model=vllm_service.config.vllm.model_name,
        messages=[
    ...
    request_dict = vllm_request.to_dict()
    
    print("3. 生成的VllmRequest:")
    print(json.dumps(request_dict, ensure_ascii=False, indent=2))
    print()
    
    # 4. 验证格式是否符合ScientificReviewer-7B要求
    print("4. 格式验证:")
    ...
    # 检查model字段
    if request_dict.get('model') == 'scientific-reviewer-7b':
        print("   ✅ model字段正确")
    else:
        print(f"   ❌ model字段错误: {request_dict.get('model')}")
    
    # 检查messages格式
    messages = request_dict.get('messages', [])
    if len(messages) >= 1:
        print("   ✅ messages字段存在且不为空")
        for i, msg in enumerate(messages):
            if 'role' in msg and 'content' in msg:
                print(f"   ✅ message {i+1} 格式正确")
            else:
                print(f"   ❌ message {i+1} 格式错误")
    else:
        print("   ❌ messages字段为空")
    
    # 检查其他字段
    if isinstance(request_dict.get('temperature'), (int, float)):
        print("   ✅ temperature字段类型正确")
    else:
        print("   ❌ temperature字段类型错误")
    
    if isinstance(request_dict.get('max_tokens'), int):
        print("   ✅ max_tokens字段类型正确")
    else:
        print("   ❌ max_tokens字段类型错误")
    ...
    # 6. 结论
    print("6. 结论:")
    if (request_dict.get('model') == 'scientific-reviewer-7b' and 
        len(request_dict.get('messages', [])) > 0 and
        all('role' in msg and 'content' in msg for msg in request_dict.get('messages', []))):
        print("   ✅ app.py生成的请求格式符合ScientificReviewer-7B要求")
        print("   ✅ 可以直接用于调用ScientificReviewer-7B模型")
    else:
        print("   ❌ app.py生成的请求格式不符合ScientificReviewer-7B要求")
        print("   ❌ 需要修改代码以适配ScientificReviewer-7B模型")
